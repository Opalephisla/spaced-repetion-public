...
export const StudyView = ({ session, onExit }) => {
  ...
  // Use ref to track the actual counts
  const statsRef = useRef({ correct: 0, reviewed: 0 });
  ...
  const handleReview = (quality) => {
    ...
    setTimeout(() => {
      if (currentIndex < session.cards.length - 1) {
        ...
        // Use ref values for final stats
        const finalStats = {
          correct: statsRef.current.correct,
          incorrect: statsRef.current.reviewed - statsRef.current.correct,
          total: statsRef.current.reviewed,
